...
from core.backbone import Backbone_Event_Offset_RT_UV
...
def train(args, TrainImgLoader, model, optimizer, scheduler, scaler, logger, device, epoch):
    ...
    for i_batch, sample in enumerate(TrainImgLoader):
        ...
        # vis_event_count_image = event_input[0, :2, ...].permute(1, 2, 0).cpu().numpy()
        # vis_event_count_image = np.concatenate((np.zeros([vis_event_count_image.shape[0], vis_event_count_image.shape[1], 1]), vis_event_count_image), axis=2)
        # cv2.imwrite(f"./visualization/{i_batch:05d}_count.png", (vis_event_count_image / np.max(vis_event_count_image) * 255).astype(np.uint8))
        # vis_event_time_image = event_input[0, 2:, ...].permute(1, 2, 0).cpu().numpy()
        ...
        # # cv2.imwrite(f"./visualization/{i_batch:05d}_time.png", (vis_event_time_image / np.max(vis_event_time_image) * 255).astype(np.uint8))
        ...
        optimizer.zero_grad()
        flow_preds, offsets_R, offsets_T, _ = model(lidar_input, event_input, iters=args.iters)
        loss = 0.0
        n_predictions = len(flow_preds)
        for i in range(n_predictions):
            i_weight = args.gamma ** (n_predictions - i - 1)
            flow_gt = data_generate.push_flow(event_frame, pc, T_err, R_err, x_list, y_list, device, offsets=[offsets_R[i], offsets_T[i]], split='train')
...
def test(args, TestImgLoader, model, device, cal_pose=False):
    ...
    for i_batch, sample in enumerate(TestImgLoader):
        ...
        _, flow_up, offset_R, offset_T, uncertainty_map, offset_flow = model(lidar_input, event_input, iters=24, test_mode=True)

        flow_image = flow_to_image(offset_flow.permute(0, 2, 3, 1).cpu().detach().numpy()[0])
        ...
        epe = torch.sum((flow_up.cpu() - flow_gt) ** 2, dim=1).sqrt()
        mag = torch.sum(flow_gt ** 2, dim=1).sqrt()
        epe = epe.view(-1)
        mag = mag.view(-1)
        ...
        if args.render:
            ...
            errors = torch.sum((flow_up.cpu() - flow_gt) ** 2, dim=1).sqrt() * valid_gt
            errors_visp = 255 - (errors - torch.min(errors)) / (torch.max(errors) - torch.min(errors)) * 255
            errors_visp = errors_visp * valid_gt
            errors_visp = torch.cat((errors_visp, torch.zeros([2, errors_visp.shape[1], errors_visp.shape[2]])), dim=0).permute(1, 2, 0).cpu().detach().numpy()[:, :, [2, 1, 0]]
...
if __name__ == '__main__':
    ...
    model = torch.nn.DataParallel(Backbone_Event_Offset_RT_UV(args), device_ids=args.gpus)
