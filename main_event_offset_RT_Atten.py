...
from core.backbone import Backbone_Event_Offset_RT_Atten
...
def train(args, TrainImgLoader, model, optimizer, scheduler, scaler, logger, device, epoch):
    ...
    for i_batch, sample in enumerate(TrainImgLoader):
        ...
        cv2.imwrite(f"./visualization/{i_batch:05d}_depth.png", (vis_lidar_input / np.max(vis_lidar_input) * 255).astype(np.uint8))

        # dilation
        edge_input = []
        for i in range(lidar_input.shape[0]):
            sparse_depth_denoise = remove_noise(lidar_input[i, :, :, :], radius=5, threshold=5)
            depth_img = sparse_depth_denoise[0, :, :].cpu().numpy() * 10.
            depth_img_dilate = sparse_to_dense(depth_img.astype(np.float32), max_depth=10)
            _, dense_edge = enhanced_depth_line_extract(depth_img_dilate / 10. * 255)
            edge_input.append(dense_edge.unsqueeze(0))
        edge_input = torch.stack(edge_input).float().to(device)
        cv2.imwrite(f'./visualization/{i_batch:05d}_edge.png', (edge_input[0, 0, ...].cpu().detach().numpy() * 255).astype(np.uint8))
        
        optimizer.zero_grad()
        flow_preds, offsets_R, offsets_T, event_fmap = model(lidar_input, event_input, edge_input, iters=args.iters)
        loss = 0.0
        ...
        lidar_fmap = model(lidar_input_gt, event_input, edge_input, encode_only=True)
...
def test(args, TestImgLoader, model, device, cal_pose=False):
    ...
    for i_batch, sample in enumerate(TestImgLoader):
        ...
        _, flow_up, offset_R, offset_T = model(lidar_input, event_input, edge_input, iters=24, test_mode=True)
...
if __name__ == '__main__':
    ...
    model = torch.nn.DataParallel(Backbone_Event_Offset_RT_Atten(args), device_ids=args.gpus) 
